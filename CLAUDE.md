...
| Pianificazioni (timer) | **implementate** in lib+CLI (`add_task`/`list_tasks`/`delete_task` in `cloud.py`; comando `task list\|add\|del`), coperte da test offline. Il "muro `tfb`" **NON esiste**: `tfb` è **AES-128 standard** (provato dal disassembly di `libNetworkAPI.so`), e il formato del task viene dalla **SDK JS ufficiale** (`broadlink-jssdk`): comando `dev_taskadd` via `sdkcontrol`, giorni = **lista `repeat` 1..7** (con shift se il +8 supera mezzanotte), orari **UTC+8** (`tasks.py`), azione = normale comando di controllo `{params,vals}`. 5 tipi: once/delay/period/cycle/random. **Da confermare su HW** l'envelope esatto della direttiva `dev_taskadd` (`KLIMAKONTROL_DEBUG=1`). Vedi memory `klimakontrol-tfb-is-aes`. (Storia: era dato per bloccato dal cifrario proprietario — sbagliato.) |
...
| App Android | **funzionante**: Kotlin + Jetpack Compose (sistema "Quadrante"), compila in **CI** (`.github/workflows/android.yml`). Fa **login** (con **selettore regione/vendor**) e **registrazione nuovo account** (due passi, come la lib), e **controlla i climatizzatori veri** via cloud (accensione, temperatura, modalità, ventola, eco/turbo/notte, **oscillazione** `ac_vdir`/`ac_hdir`), con comandi ottimistici e roll-back; sessione persistente senza password; **icona app** adattiva. In più: **Impostazioni** (cambio nickname `modifynickname` e password `modifypwd`, ricostruiti dal dex; versione app; controllo aggiornamenti via GitHub Releases; logout/forget), barra **Rinfresca casa/Spegni tutte** in Home. **Bilingue** (italiano default + inglese) con **selettore lingua** in Impostazioni (risorse `res/values`+`values-en`; switch a runtime via `LocaleStore`+`attachBaseContext`, senza AppCompat). Versione `0.5.0`. Pubblicazione Play: tutto documentato in `docs/play-store.md` (firma/AAB/CI da applicare col keystore). Espone solo i 10 del set fisso reale (§5, trappola 4): tolti mute/salute/display perché non gestiti. In più: **wizard di onboarding** (Punto 4) — aggiungi un climatizzatore vergine dall'app (config SoftAP: `data/softap/SoftApClient.kt` porta il pacchetto ricostruito, socket legato alla rete del modulo; `ui/OnboardingScreen.kt` a 4 passi, coerente col Quadrante; entry point da Home e Impostazioni). Copre la config SoftAP; per un modulo **nuovo** manca ancora il **bind in Kotlin** (portare `bind_device`) e la conferma end-to-end è **HW-gated** (Punto 3, vedi `docs/onboarding-hw-test.md`). In più: **pianificazioni (timer)** dal Dettaglio (icona orologio) — timer ricorrenti on/off (ora, giorni, temperatura), `ui/TimerScreen.kt` + `data/tasks/Timer.kt` (riusa il path `sdkcontrol` con `dev_taskadd`; formato dalla SDK JS, come la libreria Python); **envelope `dev_taskadd` da confermare su HW**. Provato su HW il 2026-08-21/22 (wizard e timer: non ancora). Vedi `android/README.md` |
