...
| Onboarding modulo — bind cloud | **implementato** in lib+CLI (`bind_device` + comando `bind`), ricostruito dal dex (`BLFamilyManager.addEndpoint` → `POST /appsync/group/dev/manage?operation=add`, corpo JSON **in chiaro**, chiave AES nel `cookie`; `familyId` da `getfamilyid`), coperto da test offline; **non ancora provato su HW** (forma della risposta da confermare con `KLIMAKONTROL_DEBUG=1`). È il **Punto 1** dell'onboarding di un modulo vergine; manca la config **SoftAP** (Punto 2: pacchetto in `libNetworkAPI.so`, ricostruibile — **non** è il muro `tfb`). Design/piano in `docs/superpowers/specs\|plans/2026-08-25-onboarding-*` |
